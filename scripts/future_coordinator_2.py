...
class Coordinator(Node):
    ...
    # Cada vez que avanza un punto en la trayectoria se notifica
    def check_ugv_reached(self):
        if self.current_ugv_wp >= self.final_wp and self.N_lookahead == 0: # Para ventana N=0
            self.ugv_reached = True
            return True
        
        if self.current_ugv_wp > self.last_wp_ugv and self.N_lookahead > 0: # Para ventana deslizante recalcular cada vez que se avanza un punto
            self.last_wp_ugv = self.current_ugv_wp
            self.ugv_reached = True
            return True

    def check_uav_reached(self):
        if self.current_uav_wp >= self.final_wp and self.N_lookahead == 0: # Para ventana N=0
            self.uav_reached = True
            return True
        
        if self.current_uav_wp > self.last_wp_uav and self.N_lookahead > 0: # Para ventana deslizante recalcular cada vez que se avanza un punto
            self.last_wp_uav = self.current_uav_wp
            self.uav_reached = True
            return True
    ...
    def vel_sync(self):
        ...
        if self.current_uav_wp >= self.current_ugv_wp:
            ...
            if(self.N_lookahead > 0 and self.current_uav_wp<len(self.waypoints_uav)): self.final_wp = self.current_uav_wp + self.N_lookahead

        elif(self.current_ugv_wp > self.current_uav_wp):

            if(self.N_lookahead == 0): self.final_wp = self.current_ugv_wp
            if(self.N_lookahead > 0 and self.current_ugv_wp<len(self.waypoints_ugv)): self.final_wp = self.current_ugv_wp + self.N_lookahead

        if self.final_wp >= len(self.waypoints_ugv) or self.final_wp >= len(self.waypoints_uav):
            ...
            if(self.N_lookahead > 0 and self.current_uav_wp == self.final_wp and self.current_ugv_wp == self.final_wp):
                self.get_logger().info("Misión completada. Todos los waypoints han sido procesados.")
                self.v_uav_xy = self.v_min
                self.v_uav_z = self.v_min
                return
    ...
    def control_loop(self):
        ...
        if self.coordinate:
            ...
            # UAV control
            if self.uav_reached:
                    self.get_logger().info("UAV. Cálculo de velocidades al WP de sync -> NUEVA sincronización")
                    self.coordinate = True
                    self.uav_reached = False
            else:
                self.uav_speed_pub.publish(Float32(data=self.v_uav_xy))
            ...
            # UGV control
            if self.ugv_reached:
                    self.get_logger().info("UGV. Cálculo de velocidades al WP de sync -> NUEVA sincronización")
                    self.coordinate = True
                    self.ugv_reached = False
            else:
                self.ugv_speed_pub.publish(Float32(data=self.v_ugv))
