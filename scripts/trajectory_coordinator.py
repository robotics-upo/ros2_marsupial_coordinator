...
class SyncCoordinator(Node):
    def __init__(self):
        ...
        # Publicadores
        self.uav_speed_pub = self.create_publisher(Float32, "/uav/reference_speed", 10)
        ...
        self.create_subscription(Float64MultiArray, '/cable_lengthPUB', self.cable_length_callback, 10)
        ...
        #self.timer = self.create_timer(timer_period, self.apply_virtual_force)

    def cable_length_callback(self, msg):
        if len(msg.data) >= 2:
            self.cable_length = msg.data[0]
            self.target_cable_length = msg.data[1]
        else:
    ...
    def compute_speeds(self):
        ...
        # Si el UGV va demasiado adelantado (caso raro, pero por robustez)
        ...
        # Publicar
        self.uav_speed_pub.publish(Float32(data=uav_speed))
...
# def apply_virtual_force(self, k=1.0, m_uav=1.0):

    #     if self.uav_position is None or self.ugv_position is None:
    #         self.get_logger().warn("Posiciones UAV o UGV no inicializadas aún.")
    #         return


    #     uav_position_np = np.array([
    #         self.uav_position.position.x,
    #         self.uav_position.position.y,
    #         self.uav_position.position.z
    #     ])

    #     ugv_position_np = np.array([
    #         self.ugv_position.position.x,
    #         self.ugv_position.position.y,
    #         0.0  # UGV en plano XY, Z = 0
    #     ])

    #     r_vec = uav_position_np - ugv_position_np
    #     current_length = self.cable_length

    #     error_length = current_length - self.target_cable_length

    #     r_hat = r_vec / current_length

    #     force_virtual = -k * error_length * r_hat

    #     accel_virtual = force_virtual / m_uav

    #     accel_along_cable = np.dot(accel_virtual, r_hat)

    #     v_corrected = self.nominal_uav_speed + accel_along_cable * self.dt

    #     v_corrected = max(self.min_speed, min(v_corrected, self.max_speed_uav))

    #     self.uav_speed_pub.publish(Float32(data=v_corrected))
    #     self.ugv_speed_pub.publish(Float32(data=self.nominal_ugv_speed))
